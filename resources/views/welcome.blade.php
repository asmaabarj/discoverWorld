...
<body>
    ...
    <div class="grid md:grid-cols-2 items-center gap-8 h-full px-16 py-8  ">
        <form action="/register" method="POST" class="max-w-lg max-md:mx-auto p-6">
                @csrf
            <div class="mb-10">
            ...
            <div>
                ...
                <input id="Fname" name="name" type="text" class="bg-gray-100 w-full text-sm px-4 py-3 rounded-md border border-solid outline-none" placeholder="Enter Full name" />
            </div>
            <div>
                <label class="text-sm mt-4 mb-2 block">Email</label>
            ...
            <div class="mt-10">
              <button type="submit" name="register" class="w-full shadow-xl py-3 px-4 text-sm font-semibold rounded text-white bg-blue-600 hover:bg-blue-700 focus:outline-none">
                register
              </button>
            </div>
            <p class="text-sm mt-6 text-center">Already have an account?<a href="/" class="text-black font-semibold hover:underline ml-1 whitespace-nowrap">Login here</a></p>
        </form>
